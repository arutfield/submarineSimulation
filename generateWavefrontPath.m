% generateWavefrontPath - create path using wavefront algorithm
%  inputs:
%   startPoint - beginning point of submarine
%   finishPoint - desired end point of submarine
%   map - map to navigate on
%   resolution - resolution of map
%  outputs:
%   path - resulting path from start to finish (empty if no path)
function path = generateWavefrontPath(startPoint, finishPoint, map, resolution)
     ...
     for k=-1:1
       ...
       for l=-1:1
         ...
         if pointMap(focusPoint(1)+v, focusPoint(2)+k, focusPoint(3)+l) == -1
           success = true;% found way by wavefront
